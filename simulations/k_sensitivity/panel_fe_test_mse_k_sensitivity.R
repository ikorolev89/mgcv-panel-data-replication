...
#   Rscript simulations/k_sensitivity/panel_fe_test_mse_k_sensitivity.R 10 7 500 4 paper_main 200 FALSE hetero_ar1 0.5 20
#
# Arguments:
...
#   10. basis dimension K for the penalized bam smooths; default 20
...
k_smooth <- if (length(args) >= 10) as.integer(args[[10]]) else 20L
...
if (is.na(k_smooth) || k_smooth < 3L) stop("bam smooth K must be at least 3.")
...
message("Penalized bam smooth basis dimension K: ", k_smooth)
...
fit_one_draw <- function(sim_id, dgp) {
  ...
  bam_fd_fit <- bam(
    dy ~ s(X_pair, by = L_pair, k = k_smooth),
  ...
  smooth_diagnostics <- function(fit) {
    sm <- fit$smooth[[1L]]
    ...
    c(
      ...
      edf_ceiling = ceiling,
      edf_ratio = ratio,
      binding_90 = as.numeric(ratio >= 0.90),
      binding_95 = as.numeric(ratio >= 0.95)
    )
  }
  bam_diag <- rbind(
    smooth_diagnostics(bam_fe_fit),
    smooth_diagnostics(bam_re_fit),
    smooth_diagnostics(bam_fd_fit)
  )
  ...
  base_result <- data.frame(
    ...
    K_smooth = c(rep(NA_integer_, 4L), rep(k_smooth, 3L)),
    smooth_edf = c(rep(NA_real_, 4L), bam_diag[, "smooth_edf"]),
    edf_ceiling = c(rep(NA_real_, 4L), bam_diag[, "edf_ceiling"]),
    edf_ratio = c(rep(NA_real_, 4L), bam_diag[, "edf_ratio"]),
    binding_90 = c(rep(NA_real_, 4L), bam_diag[, "binding_90"]),
    binding_95 = c(rep(NA_real_, 4L), bam_diag[, "binding_95"])
  )
...
summary_by_method <- do.call(
  ...
  lapply(split(losses, list(losses$dgp, losses$method), drop = TRUE), function(d) {
    data.frame(
      ...
      K_smooth = if (all(is.na(d$K_smooth))) NA_integer_ else unique(na.omit(d$K_smooth))[1],
      mean_edf = if (all(is.na(d$smooth_edf))) NA_real_ else mean(d$smooth_edf, na.rm = TRUE),
      mean_edf_ceiling = if (all(is.na(d$edf_ceiling))) NA_real_ else mean(d$edf_ceiling, na.rm = TRUE),
      mean_edf_ratio = if (all(is.na(d$edf_ratio))) NA_real_ else mean(d$edf_ratio, na.rm = TRUE),
      fraction_binding_90 = if (all(is.na(d$binding_90))) NA_real_ else mean(d$binding_90, na.rm = TRUE),
      fraction_binding_95 = if (all(is.na(d$binding_95))) NA_real_ else mean(d$binding_95, na.rm = TRUE)
    )
  })
)
